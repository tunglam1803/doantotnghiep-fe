...
//   const [filters, setFilters] = useState({ price: [], size: [], color: [] });
...
function Product() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      // If any filter is applied, use the filter endpoint
      ...
      if (Array.isArray(productList) && productList.length > 0) {
        ...
        setImages(imageMap); // Update the images state
      } else {
        setProducts([]);
        setTotalItems(0);
      }
  ...
  const handleFilterChange = (name, value) => {
    if (name === 'price') {
      // Remove special characters like 'đ' and trim spaces
      value = value.map((p) => p.replace(/[^\d\-]/g, '').trim());
    }
