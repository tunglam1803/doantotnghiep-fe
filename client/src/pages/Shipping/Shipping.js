...
//     try {
//       const response = await axios.get('http://192.168.189.110:8080/api/ship/all', {
...
//     try {
//       const response = await axios.post('http://192.168.189.110:8080/api/ship/save', newShipping, {
//         headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
//       });
...
//     try {
//       await axios.put(`http://192.168.189.110:8080/api/ship/update/${currentShipping.id}`, currentShipping, {
//         headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
//       });
//       setShippingList(
...
//     try {
//       await axios.put(`http://192.168.189.110:8080/api/ship/default/${id}`, null, {
//         headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
//       });
...
//     try {
//       await axios.delete(`http://192.168.189.110:8080/api/ship/delete/${id}`, {
//         headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
//       });
...
import React, { useState, useEffect } from 'react';
import axios from 'axios';
import { message, Button, Input, Modal, Table } from 'antd';
import { Link } from 'react-router-dom';
import config from '~/config'; // Import config if needed
...
const Shipping = () => {
  ...
  const [currentShipping, setCurrentShipping] = useState({ id: null, fullName: '', address: '', phone: '' });
  ...
  const handleAddShipping = async () => {
    ...
    try {
      ...
      setShippingList([...shippingList, { ...newShipping, id: response.data }]);
      setNewShipping({ fullName: '', address: '', phone: '' });
      message.success('Thêm địa chỉ thành công!');
  ...
  const handleDeleteShipping = async (id) => {
    ...
    try {
      ...
      message.error('Lỗi khi xóa địa chỉ giao hàng!');
    }
    await fetchShippingList();
  };
      ...
      <Table
        ...
        columns={[
          ...
          },
          {
            ...
            render: (text, record) => (
              <>
                <Button type="link" onClick={() => setDefaultShipping(record.id)} disabled={record.default}>
                  {record.default ? 'Mặc định' : 'Chọn làm mặc định'}
                </Button>
                <Button
                  type="primary"
                  danger
                  ...
                  onClick={() => handleDeleteShipping(record.id)}
                >
                  Xóa
                </Button>
                <Button
                  ...
                  shape="round"
                  style={{
                    backgroundColor: '#faad14', // Màu nền vàng
