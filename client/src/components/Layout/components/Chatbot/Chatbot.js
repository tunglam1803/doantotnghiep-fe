      ...
      {!isOpen ? (
        ...
        <Box className={cx('chat-box')}>
          <Box className={cx('chat-header')}>
            <Typography variant="h4" fontWeight="bold">UnetFashion</Typography>
...
//     const userMessage = { role: "user", content: input };
//     const updatedMessages = [...messages, userMessage]; // Cập nhật danh sách tin nhắn trước khi gửi API
//     setMessages(updatedMessages);
//     setInput("");
...
//       const result = await response.json(); // Chuyển JSON response thành object
//       console.log("API Response:", result);

//       if (result.response && Array.isArray(result.response)) {
//         const botMessage = { role: "assistant", content: result.response.join("") }; // Nối chuỗi
//         setMessages((prev) => [...prev, botMessage]);
//       } else {
//         console.error("Dữ liệu API không hợp lệ:", result);
//       }
...
//       {!isOpen ? (
//         <IconButton className={cx("chat-icon")} color="primary" size="large" onClick={() => setIsOpen(true)}>
//           <ChatIcon fontSize="large" /> Trò chuyện với bot
//         </IconButton>
//       ) : (
//         <Box className={cx("chat-box")}>
//           <Box className={cx("chat-header")}>
...
//           <Box className={cx("chat-messages")}>
//             {messages.length === 0 ? (
//               <Typography className={cx("chat-placeholder")}>Xin chào! Tôi có thể giúp gì cho bạn?</Typography>
//             ) : (
//               messages.map((msg, index) => (
//                 <Box key={index} className={cx(msg.role === "user" ? "chat-message-user" : "chat-message-bot")}>
//                   {msg.content}
